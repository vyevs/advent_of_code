...
func doPart1(initialMemory []int) {
	...
	c := computer{
		...
		getInput:      r.currentColor,
		produceOutput: r.move,
	}

	c.run()

	fmt.Printf("%d panels were painted at least once\n", len(r.panels))
}
...
func doPart2(initialMemory []int) {
	...
	drawRegistrationIdentifier(r.panels)
}
...
func (r *robot) currentColor() int {
	return r.panels[r.pos]
}

// the way the input is interpreted depends on r.stage
// if r.stage == 0, input is interpreted as the color to paint the current panel
// if r.stage == 1, input is interpreted as the direction to turn and move
func (r *robot) move(input int) {
	if r.stage == 0 {
		r.panels[r.pos] = input

		r.stage = 1
	} else if r.stage == 1 {
		if input == 0 {
			if r.dir == directionNorth {
				r.dir = directionWest
			} else {
				r.dir = direction(r.dir - 1)
			}

		} else if input == 1 {
			if r.dir == directionWest {
				r.dir = directionNorth
			} else {
				r.dir = direction(r.dir + 1)
			}

		} else {
			panic(fmt.Sprintf("robot got unknown input %d to move", input))
		}
...
func (r *robot) stepInDir() {
	...
	case directionWest:
		r.pos.x--
	default:
		panic(fmt.Sprintf("robot has unknown direction %d", r.dir))
	}
}

func drawRegistrationIdentifier(panels map[coord]int) {
	minX, minY, maxX, maxY := determineBounds(panels)

	for y := maxY; y >= minY; y-- {
		for x := minX; x <= maxX; x++ {
			if panels[coord{x: x, y: y}] == 1 {
				fmt.Print("X")
			} else {
				fmt.Print(" ")
			}
...
func determineBounds(panels map[coord]int) (minX, minY, maxX, maxY int) {
	for c := range panels {
		minX = c.x
		maxX = c.x
		minY = c.y
